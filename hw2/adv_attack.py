...
if __name__ == "__main__":
    ...
    for model_pth in ['adv_train']:    
        ...
        for batch_idx, (input, target) in enumerate(tqdm.tqdm(testloader)):
            ...
            if init_pred.item() != target.item():
                continue
            perturbed_data = pgd_attack(input, target, model, loss, args.model, epsilon, norm_mean, norm_std)
    ...
    # acc_arr=[]
    # iter_arr=[1, 2, 3, 4, 5, 10, 20, 30, 40, 80, 100] 
    ...
    #     if init_pred.item() != target.item():
    #         continue
    #     perturbed_data = pgd_attack(input, target, model, loss, args.model, epsilon, norm_mean, norm_std, niter=niter)
    #     output = model(perturbed_data)
    #     final_pred = output.max(1, keepdim=True)[1]
    #     if final_pred.item() == target.item():
    #         correct += 1
    ...
    # plt.ylabel("Accuracy")
    # plt.savefig(f'./images/Adv_Attack_Iterations_{args.model}.png')
    # plt.clf()
    ...
    # plt.xlabel("Epsilon")
    # plt.ylabel("Accuracy")
    # plt.savefig(f'./images/Adv_Attack_Epsilon_{args.model}.png')
    # plt.clf()
